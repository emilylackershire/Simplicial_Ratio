...
#This is only relevent for edge-ordered simplicial pair counting 
def smart_sort(V,E):
    E = [set(e) for e in E]
    edges_by_vertex = edge_sets(V,E)
    sorted_edges = {v:[] for v in V}
    used_list = []
    for v in V:
        for e in edges_by_vertex[v]:
            if e not in used_list:
                sorted_edges[v].append(e)
                used_list.append(e)
    return sorted_edges
